...
// One-time "this has really stalled" alert — 7h tasks overdue by another 7h, 7d tasks overdue by another 1d
async function checkOverdueMilestones(){
  if (!POCKETBASE_URL) return { error: 'POCKETBASE_URL not set' };
  const result = { checked: 0, alerted: [] };
  try{
    const filter = encodeURIComponent('done=false && overdueAlertSent=false && (duration="7h" || duration="7d")');
    const res = await fetch(`${POCKETBASE_URL}/api/collections/task_manager/records?perPage=200&filter=${filter}`);
    const data = await res.json();
    const items = Array.isArray(data.items) ? data.items : [];
    result.checked = items.length;
    ...
    for (const t of items) {
      ...
      const thresholdMs = t.duration === '7h' ? 7 * 60 * 60 * 1000 : 24 * 60 * 60 * 1000;
      const label = t.duration === '7h' ? 'another 7 hours' : '1 extra day';

      if (overdueMs >= thresholdMs) {
        await sendPushToAll({
          title: 'Board — this has really stalled',
          body: `"${t.text}" has now been overdue by ${label} beyond its original window. Worth a decision — do it or drop it.`
        });
        result.alerted.push({ id: t.id, text: t.text, duration: t.duration });
        try {
          await fetch(`${POCKETBASE_URL}/api/collections/task_manager/records/${t.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ overdueAlertSent: true })
          });
        } catch (e) {
          console.error('Failed to set overdueAlertSent', t.id, e);
        }
      }
    }
  } catch (e) {
    console.error('Overdue milestone check failed', e);
    result.error = e.message;
  }
  return result;
}

cron.schedule('*/15 * * * *', checkOverdueMilestones);

app.get('/test-overdue-milestone-check', async (req, res) => {
